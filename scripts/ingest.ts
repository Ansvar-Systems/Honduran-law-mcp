#!/usr/bin/env tsx
/**
 * Honduran Law MCP -- Census-Driven Ingestion Pipeline
 *
 * Reads data/census.json and fetches + parses every ingestable law
 * from honduras.justia.com (HTML full text).
 *
 * Pipeline per law:
 ...
 *
 * Data source: honduras.justia.com
...
import { parseHNLawHtml, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function main(): Promise<void> {
  ...
  console.log('Honduran Law MCP -- Ingestion Pipeline (Census-Driven)');
  console.log('======================================================\n');
  console.log('  Source: honduras.justia.com');
  ...
  for (const law of acts) {
    ...
    try {
      ...
      const parsed = parseHNLawHtml(html, act);
  ...
  console.log(`\n  Source:      honduras.justia.com (HTML)`);
