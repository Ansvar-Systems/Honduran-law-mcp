#!/usr/bin/env tsx
/**
 * Honduran Law MCP -- Census Script
 *
 * Scrapes honduras.justia.com to enumerate federal laws, codes, and decrees.
 * Official Honduran portals (poderjudicial.gob.hn, congreso.gob.hn) are
 * unreliable from outside Honduras, so Justia is the primary source.
 *
 * Pipeline:
 ...
 *
 * Sources:
 *   - Primary:   https://honduras.justia.com/federales/leyes/
 *   - Secondary:  https://honduras.justia.com/federales/codigos/
 *   - Tertiary:   https://honduras.justia.com/federales/decretos/
 *
 * Usage:
...
const JUSTIA_BASE = 'https://honduras.justia.com';

const USER_AGENT =
  'honduran-law-mcp/1.0 (https://github.com/Ansvar-Systems/Honduran-law-mcp; hello@ansvar.ai)';
 ...
 *
 * The /leyes/ page lists all entries on a single page (no pagination).
 */
...
async function main(): Promise<void> {
  ...
  console.log('Honduran Law MCP -- Census');
  console.log('=========================\n');
  console.log('  Primary: honduras.justia.com/federales/leyes/ (+ codigos, decretos)');
  ...
  const laws = allEntries.map((entry) => {
    const id = `hn-${entry.category}-${slugify(entry.title).substring(0, 50)}`;
  ...
  const census = {
    ...
    portal: 'honduras.justia.com',
    census_date: new Date().toISOString().split('T')[0],
    agent: 'honduran-law-mcp/census.ts',
    summary: {
  ...
  console.log('==================================================');
  console.log(`  Portal:          honduras.justia.com`);
