#!/usr/bin/env tsx
/**
 * Database builder for Honduras Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Honduras Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Honduran statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'HN');
    insertMeta.run('source', 'honduras.justia.com');
